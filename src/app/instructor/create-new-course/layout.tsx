...
import Curriculum from "@/app/instructor/create-new-course/curriculum/page"
import PublishCourse from "@/app/instructor/create-new-course/publish-course/page"
import { useRouter, usePathname } from "next/navigation"
import {Checks} from "@/assets/icons/common-icons";
...
const Layout = () => {
    ...
    const [stepsComplete, setCompleteStatus] = useState({
        step1: false,
        step2: false,
        step3: false,
        step4: false
                    ...
                    <div className="bg-white">
                        <Tabs value={currentTab} onValueChange={handleTabChange}>
                            <TabsList className="w-full grid grid-cols-4 border-b border-gray-100 [&_button]:h-16 [&_button]:justify-start">
                                <TabsTrigger value="basic-information" className={`${pathname.includes('basic-information') && '[&>svg]:text-primary-500' }`}>
                                    ...
                                    {stepsComplete.step1 && <div className="ml-auto">
                                            <Checks/>
                                        </div>}
                                </TabsTrigger>
                                <TabsTrigger value="advance-information" className={`${pathname.includes('advance-information') && '[&>svg]:text-primary-500' }`}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                ...
                                <TabsTrigger value="curriculum" className={`${pathname.includes('curriculum') && '[&>svg]:text-primary-500' }`}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            ...
                            <TabsContent value="basic-information">
                                <BasicInformation
                                    stepCompleted={() =>
                                        setCompleteStatus((prev) => ({ ...prev, step1: true }))
                                    }
                                />
                            </TabsContent>
                            ...
                            <TabsContent value="curriculum">
                                <Curriculum
                                    stepCompleted={() =>
                                        setCompleteStatus((prev) => ({ ...prev, step3: true }))
                                    }
                                />
                            </TabsContent>
                            <TabsContent value="publish-course">
                                <PublishCourse
                                    stepCompleted={() =>
                                        setCompleteStatus((prev) => ({ ...prev, step4: true }))
                                    }
                                />
                            </TabsContent>
                        </Tabs>
