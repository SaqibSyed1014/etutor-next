...
const WithdrawMoney = () => {
    ...
    return (
            <Card className="dashboard-card max-w-[536px] w-full">
                ...
                <CardContent className="p-0">
                    <div className="py-8 px-5 space-y-3">
                        ...
                        {paymentMethods.map((item, index) => {
                            return (
                                <div key={index} className={`group cursor-pointer border px-[18px] py-2 flex items-center gap-6 ${item.isActive ? 'border-success-500':'border-gray-100'}`}>
                                    <div className="w-8 grid place-items-center">{item.icon}</div>
                                    {item.message.length
                                        ? <p className="text-xs text-gray-500">{item.message}</p>
                                        :
                                        <div className={`flex items-center gap-6 flex-1 text-sm ${item.isActive ? 'text-gray-900':'text-gray-700'}`}>
                                            <span className="w-full max-w-[160px]">{item.number}</span>
                                            <span className="w-full max-w-[64px]">{item.name}</span>
                                            <span className="w-full max-w-[116px]">{item.date}</span>
                                            <div className={`${item.isActive ? 'opacity-100':'opacity-0 group-hover:opacity-100'} [&>svg]:size-6`}><CheckCircle/></div>
                                        </div>
                                    }
                                </div>
                            )
                    ...
                    <div className="flex justify-between p-5">
                        <div>
                            <div className="text-2xl text-gray-900">$16,593.00</div>
