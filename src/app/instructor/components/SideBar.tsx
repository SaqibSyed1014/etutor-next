import React from 'react';
import {BrandLogo, Stack, CreditCard} from "@/assets/icons/common-icons";
import Link from "next/link";
import {usePathname} from "next/navigation";
import {SignOut} from "@/assets/icons/icons";

const SideBar = () => {
    const pathname = usePathname();

    const sideBarLinks = [
        {
            label: 'Dashboard',
            route: '/instructor/dashboard',
            icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M4.125 19.5V12.75H9.375" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M21.375 19.5H2.625" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M9.375 19.5V8.25H14.625" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M19.875 3.75H14.625V19.5H19.875V3.75Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
        },
        {
            label: 'Create New Course',
            route: '/instructor/create-new-course',
            icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C7.02944 3 3 7.02944 3 12C3 16.9706 7.02944 21 12 21Z" stroke="currentColor" strokeWidth="1.5" strokeMiterlimit="10"/>
                <path d="M8.25 12H15.75" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M12 8.25V15.75" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
        },
        {
            label: 'My Courses',
            route: '/instructor/my-courses',
            icon: <Stack />
        },
        {
            label: 'Earning',
            route: '/instructor/earning',
            icon: <CreditCard />
        },
        {
            label: 'Message',
            route: '/instructor/chats',
            icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M4.25895 16.5939C3.14076 14.7089 2.74916 12.4805 3.15768 10.3272C3.56621 8.1739 4.74675 6.2438 6.47764 4.89933C8.20853 3.55486 10.3707 2.8885 12.5581 3.02539C14.7455 3.16227 16.8078 4.09298 18.3575 5.64274C19.9073 7.19251 20.838 9.25472 20.9749 11.4421C21.1118 13.6296 20.4455 15.7917 19.101 17.5226C17.7565 19.2535 15.8264 20.4341 13.6732 20.8426C11.5199 21.2511 9.29149 20.8596 7.40649 19.7414L7.40651 19.7413L4.29808 20.6294C4.16947 20.6662 4.03338 20.6678 3.90391 20.6343C3.77443 20.6007 3.65628 20.5332 3.5617 20.4386C3.46713 20.344 3.39956 20.2259 3.36601 20.0964C3.33246 19.9669 3.33415 19.8308 3.37089 19.7022L4.25901 16.5938L4.25895 16.5939Z" stroke="#8C94A3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M12 11.625C12.2071 11.625 12.375 11.7929 12.375 12C12.375 12.2071 12.2071 12.375 12 12.375C11.7929 12.375 11.625 12.2071 11.625 12C11.625 11.7929 11.7929 11.625 12 11.625Z" fill="#8C94A3" stroke="#8C94A3" strokeWidth="1.5"/>
                <path d="M7.5 11.625C7.70711 11.625 7.875 11.7929 7.875 12C7.875 12.2071 7.70711 12.375 7.5 12.375C7.29289 12.375 7.125 12.2071 7.125 12C7.125 11.7929 7.29289 11.625 7.5 11.625Z" fill="#8C94A3" stroke="#8C94A3" strokeWidth="1.5"/>
                <path d="M16.5 11.625C16.7071 11.625 16.875 11.7929 16.875 12C16.875 12.2071 16.7071 12.375 16.5 12.375C16.2929 12.375 16.125 12.2071 16.125 12C16.125 11.7929 16.2929 11.625 16.5 11.625Z" fill="#8C94A3" stroke="#8C94A3" strokeWidth="1.5"/>
            </svg>
        },
        {
            label: 'Settings',
            route: '/instructor/settings',
            icon: <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M12.0005 16.5C14.4858 16.5 16.5005 14.4853 16.5005 12C16.5005 9.51472 14.4858 7.5 12.0005 7.5C9.51521 7.5 7.50049 9.51472 7.50049 12C7.50049 14.4853 9.51521 16.5 12.0005 16.5Z" stroke="#8C94A3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                <path d="M11.5198 4.13952L9.75078 2.81315C9.65586 2.74175 9.54534 2.69393 9.42831 2.67363C9.31129 2.65332 9.19112 2.66112 9.0777 2.69638C8.52982 2.86794 7.99827 3.08787 7.48933 3.35355C7.38397 3.40881 7.29327 3.48834 7.22471 3.58556C7.15614 3.68278 7.11169 3.79492 7.095 3.91271L6.78228 6.10186C6.66307 6.20753 6.54637 6.31742 6.43217 6.43156C6.31801 6.54573 6.20809 6.66246 6.1024 6.78176L6.10235 6.78179L3.91359 7.09479C3.79599 7.11142 3.68402 7.15576 3.58691 7.22415C3.48981 7.29254 3.41034 7.38303 3.35507 7.48816C3.08898 7.99689 2.86863 8.52826 2.69662 9.076C2.6612 9.18957 2.6533 9.30994 2.67356 9.42717C2.69383 9.5444 2.74168 9.65513 2.81318 9.75022L4.14001 11.5193C4.13043 11.6783 4.12562 11.8385 4.12558 12C4.12558 12.1615 4.13039 12.3217 4.14002 12.4808L4.14001 12.4809L2.81364 14.2499C2.74224 14.3448 2.69442 14.4553 2.67411 14.5723C2.65381 14.6894 2.66161 14.8095 2.69686 14.923C2.86843 15.4708 3.08836 16.0024 3.35404 16.5113C3.4093 16.6167 3.48882 16.7074 3.58605 16.776C3.68327 16.8445 3.79541 16.889 3.9132 16.9057L6.10235 17.2184C6.20801 17.3376 6.31791 17.4543 6.43204 17.5685C6.54621 17.6827 6.66294 17.7926 6.78224 17.8983L6.78228 17.8983L7.09528 20.0871C7.1119 20.2047 7.15624 20.3167 7.22463 20.4138C7.29303 20.5109 7.38352 20.5903 7.48865 20.6456C7.99737 20.9117 8.52874 21.132 9.07648 21.304C9.19006 21.3395 9.31042 21.3474 9.42765 21.3271C9.54488 21.3068 9.65561 21.259 9.7507 21.1875L11.5198 19.8607C11.6788 19.8702 11.839 19.875 12.0005 19.8751C12.1619 19.8751 12.3222 19.8703 12.4813 19.8607L12.4814 19.8607L14.2504 21.187C14.3453 21.2584 14.4558 21.3063 14.5728 21.3266C14.6899 21.3469 14.81 21.3391 14.9235 21.3038C15.4713 21.1322 16.0029 20.9123 16.5118 20.6466C16.6172 20.5914 16.7079 20.5118 16.7764 20.4146C16.845 20.3174 16.8895 20.2053 16.9061 20.0875L17.2189 17.8983C17.3381 17.7927 17.4548 17.6828 17.569 17.5686C17.6831 17.4545 17.7931 17.3377 17.8988 17.2184L17.8988 17.2184L20.0876 16.9054C20.2052 16.8888 20.3171 16.8444 20.4142 16.776C20.5113 16.7076 20.5908 16.6172 20.6461 16.512C20.9122 16.0033 21.1325 15.4719 21.3045 14.9242C21.34 14.8106 21.3479 14.6902 21.3276 14.573C21.3073 14.4558 21.2595 14.3451 21.188 14.25L19.8611 12.4809C19.8707 12.3219 19.8755 12.1616 19.8756 12.0002C19.8756 11.8387 19.8708 11.6785 19.8611 11.5194L19.8611 11.5193L21.1875 9.75029C21.2589 9.65538 21.3067 9.54485 21.327 9.42783C21.3473 9.3108 21.3395 9.19063 21.3043 9.07721C21.1327 8.52933 20.9128 7.99778 20.6471 7.48884C20.5919 7.38349 20.5123 7.29279 20.4151 7.22422C20.3179 7.15566 20.2057 7.1112 20.088 7.09452L17.8988 6.78179C17.7931 6.66259 17.6832 6.54589 17.5691 6.43169C17.4549 6.31752 17.3382 6.2076 17.2189 6.10191L17.2189 6.10186L16.9059 3.9131C16.8892 3.7955 16.8449 3.68353 16.7765 3.58642C16.7081 3.48932 16.6176 3.40986 16.5125 3.35459C16.0038 3.08849 15.4724 2.86814 14.9247 2.69613C14.8111 2.66071 14.6907 2.65281 14.5735 2.67307C14.4563 2.69334 14.3455 2.7412 14.2504 2.81269L12.4814 4.13952C12.3224 4.12995 12.1621 4.12514 12.0007 4.12509C11.8392 4.12509 11.6789 4.1299 11.5199 4.13953L11.5198 4.13952Z" stroke="#8C94A3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
        }
    ]
    return (
        <aside className="bg-gray-900 w-[280px] h-screen flex flex-col gap-4">
            <div className="h-[70px] px-6 py-5 shadow-[0px_-1px_0_0_#363B47_inset] text-white">
                <BrandLogo />
            </div>
            <div className="flex flex-col">
                {sideBarLinks.map((link) => {
                    return (
                        <Link key={link.label} href={link.route} className={`sidebar-link ${pathname === link.route ? 'bg-primary-500 text-white': 'text-gray-500 hover:bg-gray-800'}`}>
                            <div className="size-6">
                                {link.icon}
                            </div>
                            <span>{link.label}</span>
                        </Link>
                    )
                })}
            </div>
            <div className="sidebar-link text-gray-500 hover:bg-gray-800 mt-auto mb-2">
                <div className="size-6">
                    <SignOut />
                </div>
                <span>Sign-out</span>
            </div>
        </aside>
    );
};

export default SideBar;
